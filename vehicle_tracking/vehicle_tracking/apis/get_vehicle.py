...
def fetch_and_update_vehicles():
    ...
    WIALON_TOKEN = Settings.wialon_token
        ...
        for data in units:
            ...
            if not any(v['name']==data['nm'] for v in doc_vehile_list):
                ...
                new_vehicle = frappe.get_doc({
                    ...
                    "last_odometer": data['lmsg']['p']['odometer'] if data.get('lmsg') is not None and data['lmsg'].get('p') and 'odometer' in data['lmsg']['p'] else 0.0,
                    "uom":"Litre",
